package com.example.admin.framelayouttest;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i=0;i<buts.length;i++){
            buts[i]=(Button)findViewById(R.id.but1+i);//15 16 17
            linears[i]=(LinearLayout)findViewById(R.id.line1+i);
            buts[i].setOnClickListener(butHandler);//핸들러 객체의 참조값들어가
        }

    }

    View.OnClickListener butHandler=new View.OnClickListener() {//멤버변수(아래에서 정의한것)
        @Override
